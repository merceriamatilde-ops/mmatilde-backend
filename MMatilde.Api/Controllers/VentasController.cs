...
public class VentasController : ControllerBase
{
    ...
    public ActionResult<object> GetTurnoSugerido([FromQuery] DateTimeOffset? fechaHora = null)
    {
        var when = fechaHora ?? DateTimeOffset.UtcNow;
        var local = VentasService.ToArgentina(when);
        return new
        {
            turno = VentasService.InferirTurno(when).ToString(),
        ...
        [FromQuery] TurnoVenta turno)
    {
        ...
        var ventas = await _db.Ventas
            .Where(v => v.Fecha >= desdeUtc && v.Fecha <= hastaUtc && v.Turno == turno)
            .ToListAsync();
